 ...
 * deve. Roda: `npm test -w @pixel-idle/forge`.
 */
...
async function main() {
  ...
  // ── fixture: character walk válido (192×192, 4f×4d) ──────────────────────
  const validWalk = path.join(DIR, "valid.walk.png");
  await makePng(validWalk, 192, 192, (set) => {
    for (let dy = 0; dy < 4; dy++)
      for (let dx = 0; dx < 4; dx++)
        for (let y = 8; y < 40; y++)
          for (let x = 12; x < 36; x++) {
            const c = PAL[(x + y) % PAL.length];
            set(dx * 48 + x, dy * 48 + y, c[0], c[1], c[2]);
          }
  });
  ...
  check("  → detecta 4 frames × 4 dir", rValid.info.frames === 4 && rValid.info.directions === 4);
  check("  → scale 1×, sem AA", rValid.info.pixelScale === 1 && rValid.info.partialAlpha === 0);

  // ── fixture: tamanho errado (50×50 opaco) ────────────────────────────────
  ...
  // ── fixture: anti-aliasing (borda alpha parcial) ─────────────────────────
  const aa = path.join(DIR, "aa.png");
  await makePng(aa, 48, 48, (set) => {
    for (let y = 8; y < 40; y++) for (let x = 8; x < 40; x++) {
      const c = PAL[(x + y) % PAL.length];
      set(x, y, c[0], c[1], c[2]);
    }
    for (let i = 7; i <= 40; i++) {
      set(i, 7, 234, 79, 54, 128);
      set(i, 41, 234, 79, 54, 128);
      set(7, i, 234, 79, 54, 128);
      set(41, i, 234, 79, 54, 128);
    }
  });
  ...
  // ── fixture: upscale 4× (12×12 → 48×48) ──────────────────────────────────
  const up = path.join(DIR, "upscaled.png");
  await makePng(up, 48, 48, (set) => {
    for (let by = 0; by < 12; by++)
      for (let bx = 0; bx < 12; bx++) {
        const corner = (bx === 0 || bx === 11) && (by === 0 || by === 11);
        if (corner) continue; // cantos transparentes
  ...
  // ── fixture: ambiguidade de frame-size (creature 64×64) ──────────────────
  ...
  // ── normalize --snap (image-q, CIEDE2000) roda e mantém válido ───────────
  ...
  // ── buildPixiSheet gera animações por direção ────────────────────────────
  ...
  check("spritesheet Pixi: 16 frames totais + anchor no pé", Object.keys(sheet.frames).length === 16 && sheet.frames["walk.south.0"].anchor.y === 1);
